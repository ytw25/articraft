...
def run_tests() -> TestReport:
    ...
    ctx.expect_origin_distance(fan, nacelle, axes="xy", max_dist=0.003)
    ctx.expect_origin_distance(turbine, nacelle, axes="xy", max_dist=0.003)
    ...
    ctx.expect_overlap(fan, nacelle, axes="xy", min_overlap=0.180)
    ctx.expect_gap(
        turbine,
        fan,
        axis="z",
        min_gap=0.030,
        positive_elem=compressor_stack,
        negative_elem=fan_blades,
        name="compressor_stack_set_back_behind_front_fan",
    )
    ctx.expect_gap(
        turbine,
        fan,
        axis="z",
        min_gap=0.200,
        positive_elem=exhaust_cone,
        negative_elem=spinner,
        name="rear_exhaust_cone_deep_inside_nacelle",
    )
    return ctx.report()
