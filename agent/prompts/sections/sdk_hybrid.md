...
- When authoring mesh-backed visuals, use managed logical names like `mesh_from_cadquery(..., "door_panel")`; do not reason about filesystem paths. Author visual geometry only; do not author collision geometry in `sdk_hybrid`.
...
TESTING
- Use `sdk_hybrid.TestContext`, return `ctx.report()`, and let `compile_model` own the baseline sanity/QC pass. Prefer `TestContext(object_model)`; do not pass asset roots in new code.
