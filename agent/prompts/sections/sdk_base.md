...
- The harness runs collision-based overlap QC on exact collision geometry derived from visuals and will surface overlaps as a non-blocking warning by default.
  - SDK compile derives exact collision geometry from visuals automatically.
  - Overlap QC is deliberately broad and can still be noisy because it relies on exact visual-derived collisions, AABB reasoning, and mesh/primitive contact queries.
...
- Treat articulation-origin proximity as a deliberately dumb static heuristic, not proof that mounting faces are flush.
...
- Use `ctx.warn_if_part_geometry_disconnected()` as the default disconnected-geometry sensor.
...
- Use `ctx.warn_if_coplanar_surfaces(ignore_adjacent=True, ignore_fixed=True)` only when it is likely to add value for the current object.
...
- Prevent floating parts:
  - Use `warn_if_part_geometry_disconnected()` when it is a helpful sensor for disconnected subassemblies inside one part.
